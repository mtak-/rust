        ...
        let ty = {
            ...
            if !flags.flags.intersects(ty::TypeFlags::KEEP_IN_LOCAL_TCX) {
                if let Some(interner) = global_interners {
                    ...
                    let ty: Ty<'gcx> = interner.arena.borrow_mut().alloc(ty_struct);
            ...
            // Don't be &mut TyS.
            let ty: Ty<'tcx> = self.arena.borrow_mut().alloc(ty_struct);
...
impl<'tcx> CommonTypes<'tcx> {
    fn new(interners: &CtxtInterners<'tcx>) -> CommonTypes<'tcx> {
        ...
        let mk_region = |r| {
            ...
            let r = interners.arena.borrow_mut().alloc(r);
...
pub struct GlobalCtxt<'tcx> {
    global_arenas: Lock<&'tcx GlobalArenas<'tcx>>,
...
impl<'a, 'gcx, 'tcx> TyCtxt<'a, 'gcx, 'tcx> {
    pub fn alloc_generics(self, generics: ty::Generics) -> &'gcx ty::Generics {
        self.global_arenas.borrow_mut().generics.alloc(generics)
    }

    pub fn alloc_steal_mir(self, mir: Mir<'gcx>) -> &'gcx Steal<Mir<'gcx>> {
        self.global_arenas.borrow_mut().steal_mir.alloc(Steal::new(mir))
    }

    pub fn alloc_mir(self, mir: Mir<'gcx>) -> &'gcx Mir<'gcx> {
        self.global_arenas.borrow_mut().mir.alloc(mir)
    }

    pub fn alloc_tables(self, tables: ty::TypeckTables<'gcx>) -> &'gcx ty::TypeckTables<'gcx> {
        self.global_arenas.borrow_mut().tables.alloc(tables)
    }

    pub fn alloc_trait_def(self, def: ty::TraitDef) -> &'gcx ty::TraitDef {
        self.global_arenas.borrow_mut().trait_def.alloc(def)
    }
    ...
    pub fn alloc_adt_def(self,
        ...
        self.global_arenas.borrow_mut().adt_def.alloc(def)
    }
    ...
    pub fn alloc_byte_array(self, bytes: &[u8]) -> &'gcx [u8] {
        if bytes.is_empty() {
            ...
            self.global_interners.arena.borrow_mut().alloc_slice(bytes)
        }
    }
    ...
    pub fn alloc_const_slice(self, values: &[&'tcx ty::Const<'tcx>])
        ...
        if values.is_empty() {
            ...
            self.interners.arena.borrow_mut().alloc_slice(values)
        }
    }
    ...
    pub fn intern_stability(self, stab: attr::Stability) -> &'gcx attr::Stability {
        ...
        let interned = self.global_interners.arena.borrow_mut().alloc(stab);
    ...
    pub fn intern_layout(self, layout: Layout) -> &'gcx Layout {
        ...
        let interned = self.global_arenas.borrow_mut().layout.alloc(layout);
                                  ...
                                  where F: for<'b> FnOnce(TyCtxt<'b, 'tcx, 'tcx>) -> R
    {
        ...
        tls::enter_global(GlobalCtxt {
            sess: s,
            cstore,
            global_arenas: Lock::new(&arenas.global),
...
impl<'a, 'tcx> Lift<'tcx> for Ty<'a> {
    ...
    fn lift_to_tcx<'b, 'gcx>(&self, tcx: TyCtxt<'b, 'gcx, 'tcx>) -> Option<Ty<'tcx>> {
        if tcx.interners.arena.borrow_mut().in_arena(*self as *const _) {
        ...
        where F: for<'a> FnOnce(TyCtxt<'a, 'gcx, 'gcx>) -> R
    {
        syntax_pos::SPAN_DEBUG.with(|span_dbg| {
            ...
            let result = //thread::spawn(|| {
                enter(&gcx, &gcx.global_interners, f);
            //}).join();
            ...
            pub fn $method(self, v: $alloc) -> &$lt_tcx $ty {
                ...
                if !($needs_infer)(&v) {
                    if !self.is_global() {
                        ...
                        let i = ($alloc_to_ret)(self.global_interners
                                                    .arena.borrow_mut().$alloc_method(v));
                ...
                let i = ($alloc_to_ret)(self.interners.arena.borrow_mut().$alloc_method(v));
